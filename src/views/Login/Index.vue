...
const login=()=>{
    formref.value.validate( async (valid)=>{//在点击登陆后统一校验
        // console.log(valid)
        if(valid){
            await usestore.useraction(form.value.account,form.value.password)
              ElMessage({
                    type:'success',
                    message:'登录成功'
              })
              router.replace('/')
            /* dologin(form.value.account,form.value.password).then(res=>{
                console.log(res)
                if(res.code==='1'){
                    ElMessage({
                    type:'success',
